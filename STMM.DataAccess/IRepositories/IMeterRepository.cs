...
namespace STMM.DataAccess.IRepositories
{
    public interface IMeterRepository : IBaseRepository<Meter>
    {
        ...
        Task<(IEnumerable<Meter> Items, int TotalCount)> GetMetersPagedAsync(string? type, bool? isActive, bool? isAssigned, string? search, int pageNumber, int pageSize, CancellationToken ct = default);
        Task<bool> ExistsSerialNumberAsync(string serialNumber, int? excludeMeterId = null, CancellationToken ct = default);
        Task<IEnumerable<Meter>> GetUnassignedMetersAsync(string? type, CancellationToken ct = default);
