...
const IconEyeShow = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>;
...
export default function UserListAdminSystem({ navigate, addToast }) {
  ...
  const fetchUsers = async () => {
    ...
    } catch { addToast('Không thể tải danh sách tài khoản toàn hệ thống.', 'error'); }
  ...
  const handleResetPassword = async () => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`${API_BASE}/${targetUser.userId}/reset-password`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ newPassword }),
      });
      if (res.ok) {
        addToast('Đặt lại mật khẩu tài khoản thành công!', 'success');
        closeModal();
      } else {
        const err = await res.json().catch(() => ({}));
            ...
            <table className="user-table">
              ...
              <tbody>
                {users.map((user, idx) => {
                  ...
                  const avatarBg = ROLE_COLORS[roleKey] || '#8b5cf6';
                  return (
                    <tr key={user.userId}>
                      ...
                      <td>
                        <span className={`badge-role ${roleKey?.replace(/\s+/g, '-') || 'custom-role'}`}>
                          {user.roleName}
                        </span>
                      </td>
                      <td>
            ...
            <div className="modal-body">
              ...
              <div className="modal-user-card">
                <div
                  ...
                  style={{ background: ROLE_COLORS[targetUser.roleName?.toLowerCase()] || '#8b5cf6' }}
                >
      ...
      {modalType === 'reset' && targetUser && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="modal-icon-wrap info"><IconKey /></div>
              ...
              <div className="modal-confirm-input">
                ...
                <div className="pw-input-wrap" style={{ position: 'relative' }}>
                  ...
                  >
                    {showPw ? <IconEyeHide /> : <IconEyeShow />}
                  </button>
                </div>
              </div>
            </div>
