...
const VendorMyServiceDetail = ({ service, onBack, onCancelService }) => {
    return (
        <div style={{ background: 'white', minHeight: '100%', padding: '32px', borderRadius: '12px', animation: 'fadeIn 0.3s ease-out' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                ...
                <div>
                    ...
                    <span style={{ color: '#64748b', fontSize: '14px' }}>Chi tiết dịch vụ đã đăng ký</span>
                </div>
            </div>
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                        <span style={{ 
                            ...
                            {service.status === 'Active' ? 'Đang hoạt động' : service.status === 'Pending' ? 'Chờ duyệt' : 'Đã hủy'}
                        </span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
                <div style={{ background: '#f8fafc', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                    <span style={{ color: '#64748b', fontSize: '13px', display: 'block', marginBottom: '8px', fontWeight: '600', textTransform: 'uppercase' }}>Ngày đăng ký</span>
                    <strong style={{ fontSize: '18px', color: '#0f172a' }}>{new Date(service.registeredAt).toLocaleDateString('vi-VN')}</strong>
                </div>
                <div style={{ background: '#f8fafc', padding: '24px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                    <span style={{ color: '#64748b', fontSize: '13px', display: 'block', marginBottom: '8px', fontWeight: '600', textTransform: 'uppercase' }}>Gia hạn tự động</span>
                    <strong style={{ fontSize: '18px', color: service.isAutoRenew ? '#059669' : '#64748b' }}>
                        {service.isAutoRenew ? 'Có bật' : 'Tắt'}
                    </strong>
                </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <span style={{ color: '#64748b', fontSize: '14px', display: 'block', marginBottom: '8px', fontWeight: '600' }}>Chi phí dịch vụ</span>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                        <strong style={{ fontSize: '32px', color: '#0f172a', fontWeight: '800' }}>{service.price.toLocaleString()}đ</strong>
                        <span style={{ color: '#64748b', fontSize: '16px' }}>/ {service.billingCycle === 'Monthly' ? 'tháng' : 'kỳ'}</span>
                    </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                    <span style={{ color: '#64748b', fontSize: '14px', display: 'block', marginBottom: '8px', fontWeight: '600' }}>Ngày hết hạn/Gia hạn tiếp theo</span>
...
export default VendorMyServiceDetail;
